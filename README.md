...
### From Regression Coefficients to Sj

The TWFE model is estimated separately for each product. The dependent variable is the log of CPI-deflated farm-gate price, so the estimated coefficients are interpretable as approximate percentage price changes. The Lag1 coefficient (β₁) captures the average price response one month after a frost shock, conditional on province fixed effects, time fixed effects, month fixed effects, and the control variables listed above.

This conditional structure is what makes β₁ interpretable as a price sensitivity parameter rather than a simple correlation. Province fixed effects remove all time-invariant province-level confounders (soil quality, market access, structural production patterns). Time fixed effects remove common shocks that affect all provinces simultaneously (exchange rate movements, energy prices, aggregate demand shifts). Month fixed effects remove seasonal price cycles that would otherwise inflate or deflate the frost coefficient depending on the time of year.

What remains after conditioning on these layers is the within-province, within-season, within-year variation in prices that co-moves with frost exposure. β₁ (Lag1) is the estimated magnitude of that co-movement, expressed as a percentage of the deflated farm-gate price. This is what enters the risk score as Sj — multiplied by 100 to express it in percentage points, then min-max normalized across non-flagged products to produce S_norm ∈ [0, 1].

Products with negative or statistically unreliable Lag1 coefficients (Portakal, Elma) are floored at zero and flagged, because a negative sensitivity would imply that frost *reduces* prices — which is theoretically inconsistent with a supply shock in a perishable market and likely reflects inventory or harvest-timing artifacts not fully absorbed by the controls.
...
This project builds on empirical work completed in CSSM 502 (Advanced Data Analysis), where the Two-Way Fixed Effects panel model was estimated. The dashboard operationalizes those results rather than re-estimating a new model, which keeps the analytical foundation stable and the scope feasible within the capstone timeline.

The core methodological contribution is the adaptation of the IPCC Exposure-Sensitivity framework to the agricultural price transmission channel. Most applications of this framework focus on yield loss or physical damage. Here, sensitivity (S) is estimated empirically from farm-gate price responses to frost shocks — a channel not previously modeled for Turkey's non-cereal markets at the province level.

The TWFE specification addresses the proposal feedback on theoretical framing: province and time fixed effects absorb unobserved heterogeneity and common macro shocks (exchange rate, energy prices); month fixed effects control for seasonal price cycles; and Lag1 is used as the price sensitivity coefficient because the empirical signal is strongest at one month post-shock. Spatial autocorrelation is partially addressed through province fixed effects but not fully — full treatment is reserved for the thesis.

On the applied side, the project directly responds to the anticipatory gap identified in TARSİM's risk assessment process and to the World Bank CCDR (2024) recommendation to scale up agricultural early warning systems in Turkey. The dashboard is framed as a working prototype — not a production system — and uncertainty communication (confidence intervals on Sj, probabilistic risk tiers) is identified as a priority gap for the thesis phase.
